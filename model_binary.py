...
class DecoderRNN_b(nn.Module):
    ...
    def sample(self, inputs, states=None, max_len=20, threshold = 0.5):
        ...
        def lstm_rec(self,inputs, states, m_len, caption, current, threshold=0.5):
            if current!=None: caption.append(current)

            if caption and (caption[-1] == 12 or caption[-1] == 1):
                # print(clean_sentence(caption))
                yield clean_sentence(caption)
            ...
            if m_len > 0 and len(caption) < 20:
                ...
                for target in targets:
                    ...
                    yield from lstm_rec(self, inputs, states, m_len, copy.deepcopy(caption), target.item(), threshold)

        return lstm_rec(self,inputs, states, max_len, [], None, threshold=0.7)